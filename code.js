...
(() => {
  ...
  async function runOperation(domain, operation, callback) {
    const payload = await callback().catch((error) => asErrorResult(operation, error));
    postToUi({ type: "operation-result", payload: __spreadProps(__spreadValues({}, payload), { domain }) });
  }
  ...
  // src/migrations/color/index.ts
  var UNSUPPORTED_COLORS = [
    "neutral",
    "support",
    "danger",
    "info",
  ...
  function parseRemovedComponentName(componentName) {
    ...
    const nonColorTokens = removedColor ? variantTokens.filter((token) => normalizeToken(token) !== normalizeToken(removedColor)) : variantTokens;
    return {
      componentSetName,
      removedColor,
      nonColorTokens
  ...
  function getVariantPropertyOrder(componentSet) {
    const definitions = Object.entries(componentSet.componentPropertyDefinitions).filter(([, definition]) => definition.type === "VARIANT").map(([key]) => key).filter((key) => !isColorVariantPropertyName(key));
    if (definitions.length > 0) {
      return definitions;
    }
    const firstComponent = componentSet.children.find((child) => child.type === "COMPONENT");
    return (firstComponent == null ? void 0 : firstComponent.variantProperties) ? Object.keys(firstComponent.variantProperties).filter((key) => !isColorVariantPropertyName(key)) : [];
  }
  ...
  function getComponentContext(node) {
    ...
    while (current) {
      if (current.type === "COMPONENT") {
        const parentSet = ((_a = current.parent) == null ? void 0 : _a.type) === "COMPONENT_SET" ? current.parent.name : null;
        return {
          type: "COMPONENT",
          name: current.name,
  ...
  async function primeVariables() {
    ...
    return {
      ...
      message: changed ? `Primed variables: renamed ${renamedVariables.length} variable${renamedVariables.length === 1 ? "" : "s"}.` : `Variables are already prepared. No names starting with "${variablePrefix}" were found in "${targetCollection.name}".`,
      details: {
        collection: {
          id: targetCollection.id,
          key: targetCollection.key,
  ...
  function getMissingComponentSetNames(plans) {
    return new Set(
      plans.filter((plan) => plan.componentSetName !== "Unknown").map((plan) => normalizeToken(plan.componentSetName))
    );
  }
  ...
  async function applyUnsupportedVariantPlans() {
    ...
    for (const plan of plans) {
      const renameNodes = await Promise.all(
        plan.variantsToRename.map((variant) => figma.getNodeByIdAsync(variant.id).catch((error) => error instanceof Error ? error : new Error(String(error))))
      );
      for (let i = 0; i < plan.variantsToRename.length; i += 1) {
        ...
        } else if ((result == null ? void 0 : result.type) !== "COMPONENT") {
          failed.push({
            id: variant.id,
            name: variant.from,
            action: "rename",
      ...
      const removeNodes = await Promise.all(
        plan.variantsToRemove.map((variant) => figma.getNodeByIdAsync(variant.id).catch((error) => error instanceof Error ? error : new Error(String(error))))
      );
  ...
  function findTargetComponent(lookup, componentSetName, tokens) {
    ...
    const candidates = componentSet.children.filter(
      (child) => child.type === "COMPONENT" && targetPropertyValues !== null && variantMatchesPropertyValues(child, targetPropertyValues)
    );
    ...
    return {
      component: null,
      reason: candidates.length === 0 ? `Could not find a current variant matching ${formatTargetValues(targetPropertyValues)}.` : `Found ${candidates.length} possible current variants matching ${formatTargetValues(targetPropertyValues)}.`,
  ...
  function getBlockedReason(removedColor, targetMode, targetComponentResult) {
    ...
    if (!(targetComponentResult == null ? void 0 : targetComponentResult.component)) {
      reasons.push((targetComponentResult == null ? void 0 : targetComponentResult.reason) || "Could not find a matching current variant.");
    }
  ...
  async function scanMissingInstances(scope, supportModeId) {
    ...
    for (let index = 0; index < instances.length; index += 1) {
      ...
      if (isMissing) {
        ...
        plans.push({
          ...
          parentName: ((_a = instance.parent) == null ? void 0 : _a.name) || null,
          pageName: getPageName(instance),
          componentSetName: (parsed == null ? void 0 : parsed.componentSetName) || "Unknown",
          removedColor: removedColor || "unknown",
          targetModeName: (targetMode == null ? void 0 : targetMode.name) || null,
          targetModeId: (targetMode == null ? void 0 : targetMode.modeId) || null,
          targetComponentId: null,
          targetComponentName: null,
          sourceComponentName: (mainComponent == null ? void 0 : mainComponent.name) || null,
          nonColorTokens: (parsed == null ? void 0 : parsed.nonColorTokens) || [],
          status: "blocked",
          reason: getBlockedReason(removedColor, targetMode, null),
          componentContextType: componentContext == null ? void 0 : componentContext.type,
          componentContextName: componentContext == null ? void 0 : componentContext.name,
          componentContextSetName: (componentContext == null ? void 0 : componentContext.componentSetName) || void 0
        });
      }
    ...
    if (plans.length > 0) {
      ...
      for (const plan of plans) {
        const targetComponentResult = plan.componentSetName !== "Unknown" ? findTargetComponent(targetLookup, plan.componentSetName, plan.nonColorTokens) : null;
        const targetComponent = (targetComponentResult == null ? void 0 : targetComponentResult.component) || null;
        plan.targetComponentId = (targetComponent == null ? void 0 : targetComponent.id) || null;
        plan.targetComponentName = (targetComponent == null ? void 0 : targetComponent.name) || null;
        plan.targetPropertyValues = (targetComponentResult == null ? void 0 : targetComponentResult.targetPropertyValues) || void 0;
        plan.targetCandidateCount = targetComponentResult == null ? void 0 : targetComponentResult.candidateCount;
        plan.targetCandidateNames = targetComponentResult == null ? void 0 : targetComponentResult.candidateNames;
    ...
    return {
      ...
      message: plans.length === 0 ? `No missing instances found.${looseSuffix}` : `Found ${plans.length} missing instance${plans.length === 1 ? "" : "s"}: ${readyCount} ready, ${blockedCount} blocked.${looseSuffix}`,
      details: {
        scope,
        scannedInstanceCount: instances.length,
        readyCount,
    ...
    for (let start = 0; start < plans.length; start += APPLY_BATCH_SIZE) {
      ...
      for (let i = 0; i < batch.length; i += 1) {
        ...
        try {
          ...
          if ((instance == null ? void 0 : instance.type) !== "INSTANCE") {
            throw new Error("Could not find this instance.");
          }
          if ((targetComponent == null ? void 0 : targetComponent.type) !== "COMPONENT") {
  ...
  async function visitFirstBoundColor(node, depth, visit) {
    ...
    if ("fills" in node && Array.isArray(node.fills)) {
      for (let index = 0; index < node.fills.length; index += 1) {
        ...
        const variableId = fill && fill.boundVariables && fill.boundVariables.color ? fill.boundVariables.color.id : null;
        if (variableId) {
    ...
    if ("strokes" in node && Array.isArray(node.strokes)) {
      for (let index = 0; index < node.strokes.length; index += 1) {
        ...
        const variableId = stroke && stroke.boundVariables && stroke.boundVariables.color ? stroke.boundVariables.color.id : null;
        if (variableId) {
  ...
  async function findLegacyColorModeOverride(node) {
  ...
  async function scanLibraryStuckInstances(scope) {
    ...
    const newComponentSetByKey = /* @__PURE__ */ new Map();
    const colorCollectionByKey = /* @__PURE__ */ new Map();
    const importErrorByKey = /* @__PURE__ */ new Map();
    ...
    const looseSuffix = looseScan.readyCount + looseScan.reviewCount > 0 ? ` Plus ${looseCleanable} support layer${looseCleanable === 1 ? "" : "s"} to clean up${looseNeedsReviewOnly > 0 ? ` (${looseNeedsReviewOnly} need review)` : ""}.` : "";
    const supportFallbackCount = instanceSupportFallbackCount + looseScan.supportFallbackCount;
    const availableColorModes = fallbackColorCollection ? fallbackColorCollection.modes.map((m) => ({ modeId: m.modeId, name: m.name })) : [];
    const hasAnyWork = readyCount + reviewCount > 0 || looseCleanable > 0;
    return {
      createdAt: (/* @__PURE__ */ new Date()).toISOString(),
      operation,
      status: hasAnyWork ? "preview" : "noop",
      message: hasAnyWork ? `Scanned ${instances.length} instances. ${readyCount} can be updated, ${reviewCount} need review, ${blockedCount} blocked.${looseSuffix}` : `Scanned ${instances.length} instances. No stuck library instances or support color layers found.`,
      details: {
        scannedInstanceCount: instances.length,
        candidateCount: candidates.length,
        readyCount,
  ...
  var LEGACY_COLOR_VARIABLE_CATEGORY_PATTERN = /^color\/(main|support|neutral)\/(.+)$/;
  function classifyLegacyColorVariable(variableName, collectionName) {
    const match = variableName.match(LEGACY_COLOR_VARIABLE_CATEGORY_PATTERN);
    if (match) {
      return { category: match[1], lookupName: match[2] };
    }
  ...
  async function resolveLegacyColorPaintTarget(paint, newColorVariableMap, variableCache, collectionCache) {
    if (!paint || paint.type !== "SOLID") {
      return null;
    }
    const variableId = paint.boundVariables && paint.boundVariables.color ? paint.boundVariables.color.id : null;
    if (!variableId) {
      return null;
    }
    ...
    if (variable === void 0) {
      ...
      variableCache.set(variableId, variable === void 0 ? null : variable);
    }
    if (!variable) {
      return null;
    }
    ...
    if (collection === void 0) {
      ...
      collectionCache.set(variable.variableCollectionId, collection === void 0 ? null : collection);
    }
  ...
  async function applyLibraryStuckInstancePlans(supportFallbackModeId, rebindLegacyVariables) {
    ...
    for (let index = 0; index < applicablePlans.length; index += 1) {
      ...
      try {
        ...
        const effectiveModeId = plan.needsSupportModeChoice && supportFallbackModeId ? supportFallbackModeId : plan.targetModeId;
  ...
  async function getRebindScopeRoots(scope) {
    if (scope === "selection") {
  ...
  async function inspectNodeLegacyBindings(node, newColorVariableMap, variableCache, collectionCache) {
    const bindings = [];
    for (const propertyName of ["fills", "strokes"]) {
      const paintNode = node;
      const paints = paintNode[propertyName];
  ...
  async function rebindLegacyColorPaintsOnNodeOnly(node, newColorVariableMap, variableCache, collectionCache) {
    const stats = { rebound: 0, skipped: 0, failed: 0 };
    for (const propertyName of ["fills", "strokes"]) {
      const paintNode = node;
      const paints = paintNode[propertyName];
  ...
  async function collectLegacyColorRebindPlans(scope, colorCollection) {
    pendingLegacyColorRebindPlans = [];
    ...
    return {
      ...
      reboundBindingCount: plans.filter((plan) => plan.status === "ready").reduce((sum, plan) => sum + plan.matchedCount, 0)
    };
  }
  async function applyLegacyColorRebindPlans(supportFallbackModeId) {
    const empty = {
      reboundCount: 0,
      skippedCount: 0,
      failedPaintCount: 0,
    ...
    for (const plan of plans) {
      try {
        ...
        if (effectiveModeId && "setExplicitVariableModeForCollection" in sceneNode) {
          try {
            sceneNode.setExplicitVariableModeForCollection(colorCollection, effectiveModeId);
            modeSetCount += 1;
          } catch (e) {
          }
        }
        if (plan.legacyModeCollectionId && "clearExplicitVariableModeForCollection" in sceneNode) {
          try {
            const legacyCollection = await figma.variables.getVariableCollectionByIdAsync(plan.legacyModeCollectionId);
            if (legacyCollection) {
              sceneNode.clearExplicitVariableModeForCollection(legacyCollection);
            }
          } catch (e) {
          }
        }
