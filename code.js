...
let pendingLegacyColorRebindPlans = [];
...
async function applyLegacyColorRebindPlans(supportFallbackModeId) {
    ...
    const plans = pendingLegacyColorRebindPlans.filter((plan) => plan.status === 'ready' || (plan.needsSupportModeChoice && supportFallbackModeId));
    pendingLegacyColorRebindPlans = [];
